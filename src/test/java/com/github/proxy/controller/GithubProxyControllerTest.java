...
class GithubProxyControllerTest {
    ...
    void getRepositoriesInformation_ReturnsListOfUserRepositoryDetailDto() throws Exception {
        ...
        Mockito.verify(githubApiClient).getRepositories(username);
        Mockito.verify(githubApiClient).getBranches(username, "test-name");
        Mockito.verifyNoMoreInteractions(githubApiClient);
    }
